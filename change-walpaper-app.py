...
class WallpaperApp(QWidget):
    ...
    def check_feh_package(self):
        try:
            ...
            deb_path = os.path.join(os.path.dirname(sys.argv[0]), "feh.deb")
            if os.path.exists(deb_path):
                reply = QMessageBox.question(
                    self,
                    "Feh не найден",
                    "Приложение feh не установлено. Установить локальный пакет feh.deb?",
                ...
                QMessageBox.warning(
                    self,
                    "Feh не найден",
                    "Feh не установлен и локальный пакет feh.deb не найден!"
                )
